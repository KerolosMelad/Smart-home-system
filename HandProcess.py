...
class Hand_Process :  
    ...
    HandsModel = mediapipe.solutions.hands.Hands(static_image_mode=False,max_num_hands=1,min_detection_confidence=0.7, min_tracking_confidence=0.7)
    outputs=[]
    tracked_point=0
    ...
    def Hand(self,frame): #
        ...
        if(self.move == True and (time.time() - self.StarAT >=timeToRecognition) and  self.StarAT !=-1 ): # Takes 2 seconds to recognize movment 
            ...
            results = self.HandsModel.process(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))

            if results.multi_hand_landmarks != None:
                self.outputs = results.multi_hand_landmarks
                for handLandmarks in results.multi_hand_landmarks:
                    ...
                    if (thumbIsOpen and  firstFingerIsOpen and secondFingerIsOpen and thirdFingerIsOpen and fourthFingerIsOpen):
                        ...
                        if(self.frameCounter %self.FramTHreshold == 0  ):
                            ...
                            self.tracked_point= ( (int(x_center)), (int(y_center))  )
